...
instance Show TransformationAST where
    ...
    show (T (Combine [])) = "[]"
    show (T (Combine [t])) = show t
    show (T (Combine (t:ts))) = "[" ++ show t ++ helper ts ++ "]"
        where
            helper [] = ""
...
instance Show RegionAST where
    show reg = init ((foldRegionAST showHelp reg) 0)

showHelp :: RegionCombiner (Int -> String)
showHelp combiner level = case combiner of
    FromPoints points -> showIndented level $ "FromPoints " ++ show points ++ "\n"
    Rectangle width height -> showIndented level $ "Rectangle " ++ show width ++ " " ++ show height ++ "\n"
    Circle radius -> showIndented level $ "Circle " ++ show radius ++ "\n"
    Complement fct -> showIndented level "~\n" ++ fct (helperLev level)
    Union reg1 reg2 -> showIndented level "+\n" ++  (showSubRegions reg1 reg2 level)
    Intersection reg1 reg2 -> showIndented level "*\n" ++ (showSubRegions reg1 reg2 level)
    Transform transformation region -> showIndented level (show transformation ++ "\n") ++ region (helperLev level)

showSubRegions :: (Int -> String) -> (Int -> String) -> Int -> String
showSubRegions reg1 reg2 lev = reg1 (helperLev lev) ++ reg2 (helperLev lev)

showIndented :: Int -> String -> String
showIndented lev content = replicate (2 * lev) ' ' ++ content
...
basicEntityCount = foldRegionAST combiner
  where
    ...
    combiner region = case region of
      ...
      Union (region1, transformations1) (region2, transformations2) ->
         calculateSums (region1, transformations1) (region2, transformations2)
      Intersection (region1, transformations1) (region2, transformations2) ->
        calculateSums (region1, transformations1) (region2, transformations2)
      Transform transformation (regions, transformations) ->
        (regions, transformations + basicTransformationCount transformation)

    calculateSums :: (Int, Int) ->  (Int, Int) -> (Int, Int)
    calculateSums (region1Count, transformation1Count) (region2Count, transformation2Count) =
      (regionSum, transformationSum)
      where 
        regionSum = region1Count + region2Count
        transformationSum = transformation1Count + transformation2Count
