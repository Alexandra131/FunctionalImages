...
rectangle width height = 
    let maxOrigLat = width / 2
        minOrigLAt = -maxOrigLat
        maxOrigInalt = height / 2
        minOrigInalt = -maxOrigInalt
    in \(x, y) ->  x >= minOrigLAt && x <= maxOrigLat && y >= minOrigInalt &&  y <= maxOrigInalt
...
plot width height region =
    let verificareRegiune (x, y) = if region (x, y) then '*' else '.'
        linii = [[verificareRegiune (fromIntegral x, fromIntegral y) | x <- [-width .. width]] | y <- [height, height-1 .. -height]]
        rezultat = intercalate "\n" linii
    in rezultat
...
combineTransformations = foldl compunere id
  where
    compunere :: Transformation -> Transformation -> Transformation
    compunere f g = g . f
...
{-
    *** TODO ***
    ...
    Răspuns: 
        ...
        Evaluarea lenese ne ajuta pentru a nu construii toate cercurile in acelasi timp, ci pentru a le face pe rand
        pe masura ce avansam in recursivitate.
        In cazul in care noi am vrea sa verifica amapretenunta unui punct (n, 0), n sa fie orice valoare,  sa zic 3,
        iar noi am avea "circles 2" s.ar genera doar primul cerc, restul cercurilor evalunadu.se doar daca este necesar
-}
circles :: Int -> Region
circles n
    | n <= 0    = const False
...
{-
    *** TODO ***
    ...
    Răspuns: pai se vor genera cercuri la infinit pana nu o sa mai fie suficient spatiu, 
-}
...
bfs start expand = bfs' [(start, 0)] (S.singleton start)
  where
    bfs' [] _ = []
    bfs' ((curent, dist):rest) vizitati = (curent, dist) : urm
      where
        vecini = filter (\vecin -> not (S.member vecin vizitati)) (expand curent)
        urm = bfs' (rest ++ map (\x -> (x, dist + 1)) vecini) (foldl (\acc x -> S.insert x acc) vizitati vecini)
...
regionAvoidingBfs start region = bfs start expand
  where
    expand :: Point -> [Point]
    expand curent = filter (\p -> not (region p)) (vecini curent)
    vecini :: Point -> [Point]
    vecini (x, y) = [nord (x, y), est (x, y), vest (x, y), sud (x, y)]

    nord, est, vest, sud :: Point -> Point
    nord (x, y) = (x, y+1)
    est (x, y) = (x+1, y)
    vest (x, y) = (x-1, y)
    sud (x, y) = (x, y-1)